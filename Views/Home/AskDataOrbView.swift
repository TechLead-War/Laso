...
struct AskDataOrbView: View {
    ...
    private func render(ctx: GraphicsContext, size: CGSize, time t: Double) {
        let W = size.width, H = size.height
        let cx = W / 2, cy = H / 2
        let R = min(W, H) * 0.314

        // Breathing scale
        let breathe = 1 + 0.018 * sin(t * 0.9)
        var main = ctx
        main.translateBy(x: cx, y: cy)
        main.scaleBy(x: breathe, y: breathe)
        main.translateBy(x: -cx, y: -cy)

        // Glow pulse
        let glowPulse = 0.7 + 0.3 * sin(t * 1.1)

        // Layer 1: Atmosphere (pulsed opacity)
        var atmo = main
        atmo.opacity = glowPulse
        drawAtmosphere(&atmo, cx: cx, cy: cy, R: R, t: t)

        // Layer 2: Outer Shell
        drawOuterShell(&main, cx: cx, cy: cy, R: R, t: t)

        // Layer 3: Rings
        drawRings(&main, cx: cx, cy: cy, R: R, t: t)

        // Layer 4: Inner Void
        drawInnerVoid(&main, cx: cx, cy: cy, R: R, t: t)
    }

    // MARK: - Core Math (exact port)

    private func loopDisplacement(a: Double, t: Double, wobble: Double, drift: Double, rippleScale: Double) -> Double {
    ...
    private func traceLoop(cx: Double, cy: Double, radius: Double, t: Double, wobble: Double, drift: Double, detail: Int, rippleScale: Double = 0.2) -> Path {
        let TAU = Double.pi * 2
        var path = Path()
        for i in 0...detail {
    ...
    private func drawAtmosphere(_ ctx: inout GraphicsContext, cx: Double, cy: Double, R: Double, t: Double) {
        ...
        // Center void (back to normal blend, clipped context)
    ...
    private func drawOuterShell(_ ctx: inout GraphicsContext, cx: Double, cy: Double, R: Double, t: Double) {
        let TAU = Double.pi * 2
        var shell = ctx
        ...
        for s in shells {
            for rib in 0..<s.ribs {
                ...
                for i in 0..<s.points {
                    ...
                    let rect = Path(CGRect(x: x - sz / 2, y: y - sz / 2, width: sz, height: sz))
                    shell.fill(rect, with: .color(rgba(s.r, s.g, s.b, alpha)))
                }
            }
        }
    }
    ...
    private func drawRings(_ ctx: inout GraphicsContext, cx: Double, cy: Double, R: Double, t: Double) {
        ...
        // Second ring core
        ctx.stroke(secondPath, with: .linearGradient(
            Gradient(stops: [
        ...
        // Traveling hotspots
        let hotA = t * 0.42 + 2.24, hotB = t * 0.42 + 5.14
        let h1 = CGPoint(x: cx + cos(hotA) * R * 0.985, y: cy + sin(hotA) * R * 0.985)
        let h2 = CGPoint(x: cx + cos(hotB) * R * 0.985, y: cy + sin(hotB) * R * 0.985)
    ...
    private func drawInnerVoid(_ ctx: inout GraphicsContext, cx: Double, cy: Double, R: Double, t: Double) {
        ...
        let eCenter = CGPoint(x: cx - R * 0.15 + driftX * 0.5, y: cy - R * 0.11 + driftY * 0.4)
