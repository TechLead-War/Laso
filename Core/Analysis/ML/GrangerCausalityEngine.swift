...
struct GrangerCausalityEngine {
    ...
    static func regularizedIncompleteBeta(x: Double, a: Double, b: Double) -> Double {
        ...
        // Split into sub-expressions so Swift's type checker can resolve in time.
        let powerTerm: Double = a * log(x) + b * log(1.0 - x)
        let betaLog: Double = lgamma(a) + lgamma(b) - lgamma(a + b)
        let logPrefactor: Double = powerTerm - log(a) - betaLog
