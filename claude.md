...
13. **Concise responses — HARD LIMIT (3 lines max, usually 1 line)** — Default to a one-line answer. Absolute cap is 3 lines total, excluding the confidence line. No headers, no bullet lists, no tables, no multi-section breakdowns, ever, unless the user explicitly says "explain", "break it down", "show options", or "show code". If the answer cannot fit in 3 lines, ask "short or detailed?" first. Long replies get ignored and waste the user's time.
